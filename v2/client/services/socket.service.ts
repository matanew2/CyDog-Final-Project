// At the top of the file, add this check for preview/development mode
const IS_PREVIEW =
...
class SocketService {
  ...
  private authenticate(token: string): Promise<void> {
    // In preview mode, simulate successful authentication
    if (IS_PREVIEW) {
  ...
  public send(type: string, payload: any): void {
    ...
    if (IS_PREVIEW) {
      ...
      if (type === SocketEvent.DOG_COMMAND) {
        setTimeout(() => {
          this.emit(SocketEvent.DOG_COMMAND_RESPONSE, {
            dogId: payload.dogId,
