...
import { Dog, Users, ClipboardList, LogOut, Settings, Bell, Moon, Sun, ChevronLeft, ChevronRight } from "lucide-react"
      ...
      {user && !collapsed && (
        <div className="flex items-center gap-3 p-4 border-b border-teal-800 dark:border-slate-800">
          <Avatar className="h-10 w-10 border border-teal-500">
            ...
            <AvatarFallback className="bg-teal-700 dark:bg-teal-800">{user.name?.[0] || "U"}</AvatarFallback>
          </Avatar>
          <div className="flex-1 min-w-0">
            <p className="font-medium truncate">{user.name || "User"}</p>
            <p className="text-xs text-teal-100 dark:text-slate-300 truncate">{user.email}</p>
          </div>
        </div>
      )}
                ...
                <TooltipProvider>
                  <Tooltip>
                    <TooltipTrigger asChild>
                      ...
                      >
                        <item.icon
                          className={`${collapsed ? "" : "mr-3"} h-6 w-6 transition-transform duration-300 ${
                        ...
                        {isActive && (
                          <motion.div
                            ...
                            className={`absolute ${
                              collapsed ? "right-0 left-0 bottom-0 h-1 w-full rounded-t-none" : "right-0 h-8 w-1"
              ...
              <TooltipTrigger asChild>
                ...
                >
                  ...
                  {!collapsed && (
                    <motion.span
                      initial={false}
                      animate={isHovered === "logout" ? { x: 5, opacity: 1 } : { x: 0, opacity: 0.9 }}
                      transition={{ duration: 0.2 }}
                    >
                      Logout
                    </motion.span>
