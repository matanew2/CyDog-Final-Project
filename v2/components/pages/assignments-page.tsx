...
export function AssignmentsPage() {
  ...
  const filteredAssignments = assignments.filter((assignment) => {
    ...
    return (
      ...
      (handler && handler.name.toLowerCase().includes(searchTerm.toLowerCase()))
    )
  })
      ...
      <div className="container mx-auto p-6">
        ...
        <Tabs defaultValue="active" className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-teal-900/50">
            <TabsTrigger value="active" className="data-[state=active]:bg-teal-700">
              Active ({activeAssignments.length})
            </TabsTrigger>
            <TabsTrigger value="pending" className="data-[state=active]:bg-teal-700">
              Pending ({pendingAssignments.length})
            </TabsTrigger>
            <TabsTrigger value="completed" className="data-[state=active]:bg-teal-700">
                ...
                <CardContent>
                  ...
                  {assignment.description && (
                    <div className="mt-4 rounded-lg bg-teal-900/30 p-3">
                      <p className="mb-1 text-sm font-medium text-teal-100">Description</p>
                  ...
                  <div className="mt-4 flex gap-2">
                    ...
                    {assignment.status === "Active" && (
                      <Button className="flex-1 bg-blue-600 hover:bg-blue-500">Complete</Button>
                    )}
                    {assignment.status === "Pending" && (
                      <Button className="flex-1 bg-green-600 hover:bg-green-500">Start</Button>
                    )}
                  </div>
                </CardContent>
              </Card>
