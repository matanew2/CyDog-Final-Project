...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Viewport
    ref={ref}
    className={cn(
      "fixed bottom-0 right-0 z-[9999] flex max-h-screen w-full flex-col-reverse p-4 sm:bottom-0 sm:right-0 sm:flex-col md:max-w-[420px]",
      className
    )}
    {...props}
  />
...
const Toast = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<typeof ToastPrimitives.Root> &
    VariantProps<typeof toastVariants> & {
      /**
       * The duration in milliseconds that the toast should remain visible.
       * @default 3000
       */
      duration?: number;
      /**
       * Show progress bar to indicate toast timeout
       * @default true
       */
...
>(
  (
    { className, variant, duration = 3000, showProgress = true, ...props },
    ref
  ) => {
    ...
    React.useEffect(() => {
      ...
      const updateProgress = () => {
        if (isPaused) {
          // If paused, only update lastUpdateTime
        ...
        setRemainingTime((prevRemaining) => {
          const newRemaining = Math.max(
            0,
            prevRemaining - elapsedSinceLastUpdate
          );

          // Auto-dismiss when time is up
          if (newRemaining <= 0 && closeRef.current) {
    ...
    return (
      <ToastPrimitives.Root
        ...
        {...props}
      >
        <ToastPrimitives.Close ref={closeRef} className="hidden" />
        {props.children}
        {showProgress && (
          <div className="absolute bottom-0 left-0 h-1.5 w-full overflow-hidden bg-transparent">
            <div
